...
fn is_declared_dds_address(addr: &Vec<u8>, ordinal: u64, dds_store: &store::StoreGetInt64) -> bool {
    // 使用Result处理hex::decode可能的错误
    let hardcoded_address = hex::decode("88e6A0c2dDD26FEEb64F039a2c41296FcB3f5640").unwrap_or_default();

    if addr == &hardcoded_address {
        substreams::log::info!("Checking if address {} is declared dds address", hex::encode(addr));
        return true;
    }

    // 使用hex::encode而不是Hex自定义类型
    if dds_store.get_at(ordinal, hex::encode(addr)).is_some() {
        return true;
    }
